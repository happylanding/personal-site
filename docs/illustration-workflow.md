...
> ## ⛔ 已废弃（2026-08-21）
>
> 由 Galvin 在 Issue #153 明确要求：**所有文章上传/发布过程中不再生成任何配图**。
> 前期已生成的全部配图已删除（`public/images/` 清空），`ogImage`/`ogImageEn` 已从文章 frontmatter 移除，站点封面/社交分享图回退到默认 `/favicon.svg`。
> 本文件仅作历史存档，**不再执行**。相关生成脚本已弃用。
...
| 阅读思考 | `books` | 紫罗兰 `#7C3AED` | 品红 `#F33BD6` | `#A78BFA` |
...
| 阅读思考 | `books` | `#7C3AED` | `#F33BD6` | `#A78BFA` |
