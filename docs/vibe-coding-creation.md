...
【但其实这样的对话方式存在大量算力浪费的风险，很明显不符合要求这样的描述会让它冲核心去联系上下文，重新检查很多本不必要的内容，在项目部分细节优化的时候，存在这样的问题，仅仅解决一个文本溢出的问题，就消耗了100多算力，还没解决好，这玩意儿就根本不值啊！！！但后来我逐步调整思路，冷静下来跟他详细描述，也还算可以吧。。。】
...
每一个问题都是我截图发给 AI，AI 分析 → 修改 → 构建 → 用 Playwright 在 iPhone 17 模拟器下截图验证。没有一次是"拍脑门改一下试试"——每一步都有根因分析和验证手段。
...
最终线上地址：**https://personal-site-6ed.pages.dev**（55+ 个双语页面，构建零错误）

后面由于该域名比较难记，我们尝试了在项目中修改子域名的方式，失败了。最终AI给出的方案是重新部署一遍项目，直接吧名称换成galvinai即可，这下是解决了。
...
- 给出修改方案并直接执行【很多时候我会要求它先出一个方案，经过人工确认可行后才能执行】
- 构建项目
...
**方案**：了解到这个机制后，接受了当前域名，把更多精力放回内容本身。
